            ...
            <div class="card shadow">
                <div class="card-header border-0">
                 ...
                 <div class="card-body">
                    ...
                    @endif
                    <form method="post" enctype="multipart/form-data" action="{{ route('admin.collages.gallery.update') }}" autocomplete="off">
                        @csrf
                        <input type="hidden" name="id" value="{{ $gallery->id }}" />
                         <div class="pl-lg-4">
                            <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                <label class="form-control-label" for="input-name">{{ __('Name') }}</label>
                                <input type="text" name="name" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Name') }}" value="{{ $gallery->name }}" required autofocus>
